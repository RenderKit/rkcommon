...
#define ScalarT typename L::Vector::Scalar
    ...
    struct AffineSpaceT
    {
      ...
      inline AffineSpaceT(const AffineSpaceT &other)
      {
        l = other.l;
        p = other.p;
      }
      inline AffineSpaceT(const L &other)
      {
        l = other;
        p = VectorT(zero);
      }
      inline AffineSpaceT &operator=(const AffineSpaceT &other)
      {
        l = other.l;
        p = other.p;
        return *this;
      ...
      inline AffineSpaceT(const VectorT &vx,
                          const VectorT &vy,
                          const VectorT &vz,
                          const VectorT &p)
          : l(vx, vy, vz), p(p)
      ...
      inline AffineSpaceT(const AffineSpaceT<L1> &s) : l(s.l), p(s.p)
      {
      }
      ...
      static inline AffineSpaceT scale(const VectorT &s)
      {
        return L::scale(s);
      }

      /*! return matrix for translation */
      static inline AffineSpaceT translate(const VectorT &p)
      {
        return AffineSpaceT(one, p);
      }

      /*! return matrix for rotation, only in 2D */
      static inline AffineSpaceT rotate(const ScalarT &r)
      {
        return L::rotate(r);
      }

      /*! return matrix for rotation around arbitrary point (2D) or axis (3D) */
      static inline AffineSpaceT rotate(const VectorT &u, const ScalarT &r)
      {
        return L::rotate(u, r);
      }
       ...
       * coordinate system */
      static inline AffineSpaceT lookat(const VectorT &eye,
                                        const VectorT &point,
                                        const VectorT &up)
      {
    ...
    template <typename L>
    inline AffineSpaceT<L> operator-(const AffineSpaceT<L> &a)
    {
    ...
    template <typename L>
    inline AffineSpaceT<L> rcp(const AffineSpaceT<L> &a)
    {
      L il = rcp(a.l);
    ...
    template <typename L>
    inline AffineSpaceT<L> operator/(const AffineSpaceT<L> &a, const ScalarT &b)
    {
      return a * rcp(b);
    }
    ...
    template <typename L>
    inline AffineSpaceT<L> &operator*=(AffineSpaceT<L> &a, const ScalarT &b)
    {
      return a = a * b;
    }
    template <typename L>
    ...
    template <typename L>
    inline const VectorT xfmPoint(const AffineSpaceT<L> &m, const VectorT &p)
    {
      return madd(VectorT(p.x),
                  m.l.vx,
                  madd(VectorT(p.y), m.l.vy, madd(VectorT(p.z), m.l.vz, m.p)));
    }
    template <typename L>
    inline const VectorT xfmVector(const AffineSpaceT<L> &m, const VectorT &v)
    {
      return xfmVector(m.l, v);
    }
    template <typename L>
    inline const VectorT xfmNormal(const AffineSpaceT<L> &m, const VectorT &n)
    {
      return xfmNormal(m.l, n);
    }

    template <typename S, bool A = false>
    inline const box_t<S, 3, A> xfmBounds(
        const AffineSpaceT<LinearSpace3<vec_t<S, 3, A>>> &m,
        const box_t<S, 3, A> &b)
    {
      box_t<S, 3, A> dst = empty;
      const vec_t<S, 3, A> p0(b.lower.x, b.lower.y, b.lower.z);
      dst.extend(xfmPoint(m, p0));
      const vec_t<S, 3, A> p1(b.lower.x, b.lower.y, b.upper.z);
      dst.extend(xfmPoint(m, p1));
      const vec_t<S, 3, A> p2(b.lower.x, b.upper.y, b.lower.z);
      dst.extend(xfmPoint(m, p2));
      const vec_t<S, 3, A> p3(b.lower.x, b.upper.y, b.upper.z);
      dst.extend(xfmPoint(m, p3));
      const vec_t<S, 3, A> p4(b.upper.x, b.lower.y, b.lower.z);
      dst.extend(xfmPoint(m, p4));
      const vec_t<S, 3, A> p5(b.upper.x, b.lower.y, b.upper.z);
      dst.extend(xfmPoint(m, p5));
      const vec_t<S, 3, A> p6(b.upper.x, b.upper.y, b.lower.z);
      dst.extend(xfmPoint(m, p6));
      const vec_t<S, 3, A> p7(b.upper.x, b.upper.y, b.upper.z);
    ...
    template <typename L>
    inline bool operator==(const AffineSpaceT<L> &a, const AffineSpaceT<L> &b)
    {
    ...
    {
      return translate(+p) * AffineSpace2f(LinearSpace2f::rotate(r)) *
             translate(-p);
    }
