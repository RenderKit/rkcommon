  ...
  namespace math {
    ...
    __forceinline float signmsk(const float x)
    {
      return _mm_cvtss_f32(_mm_and_ps(
          _mm_set_ss(x), _mm_castsi128_ps(_mm_set1_epi32(0x80000000))));
    }
    ...
    {
      return _mm_cvtss_f32(
          _mm_and_ps(_mm_set_ss(x), _mm_castsi128_ps(_mm_set1_epi32(y))));
    }
    ...
    {
      return x * T(5.7295779513082320876798154814105e1);
    }
    template <typename T>
    ...
    {
      return uint32_t(rand()) ^ (uint32_t(rand()) << 8) ^
             (uint32_t(rand()) << 16);
    }
#else
    template <>
    ...
    template <typename T>
    __forceinline T lerp2(const float x0,
                          const float x1,
                          const float x2,
                          const float x3,
                          const T &u,
                          const T &v)
    {
      return (1.0f - u) * (1.0f - v) * x0 + u * (1.0f - v) * x1 +
             (1.0f - u) * v * x2 + u * v * x3;
    }
