...
namespace ospcommon {
  ...
  void setAffinity(HANDLE thread, ssize_t affinity)
  {
    ...
    typedef BOOL(WINAPI * SetThreadGroupAffinityFunc)(
        HANDLE, const GROUP_AFFINITY *, PGROUP_AFFINITY);
    typedef BOOL(WINAPI * SetThreadIdealProcessorExFunc)(
    ...
    SetThreadGroupAffinityFunc pSetThreadGroupAffinity =
        (SetThreadGroupAffinityFunc)GetProcAddress(hlib,
                                                   "SetThreadGroupAffinity");
    SetThreadIdealProcessorExFunc pSetThreadIdealProcessorEx =
        (SetThreadIdealProcessorExFunc)GetProcAddress(
            hlib, "SetThreadIdealProcessorEx");
    if (pGetActiveProcessorGroupCount && pGetActiveProcessorCount &&
        pSetThreadGroupAffinity && pSetThreadIdealProcessorEx &&
      ...
      if (!pSetThreadIdealProcessorEx(thread, &processorNumber, nullptr))
        WARNING(
            "SetThreadIdealProcessorEx failed");  // on purpose only a warning
    } else {
  ...
  thread_t createThread(thread_func f,
                        void *arg,
                        size_t stack_size,
                        ssize_t threadID)
  {
    HANDLE thread = CreateThread(nullptr,
                                 ...
                                 new ThreadStartupData(f, arg),
                                 0,
                                 nullptr);
    if (thread == nullptr)
      throw std::runtime_error("ospcommon::CreateThread failed");
    if (threadID >= 0)
...
namespace ospcommon {
                        ...
                        ssize_t threadID)
  {
    ...
    if (pthread_create(tid,
                       &attr,
                       (void *(*)(void *))threadStartup,
                       new ThreadStartupData(f, arg, threadID)) != 0)
