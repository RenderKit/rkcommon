...
#define PING                                                   \
  {                                                            \
    ...
    msg << CODE_LOCATION << ": " << __FUNCTION__ << std::endl; \
    std::cout << msg.str();                                    \
  }
#define PRINT(x)                                   \
  {                                                \
    std::stringstream msg;                         \
    msg << STRING(x) << " = " << (x) << std::endl; \
    std::cout << msg.str();                        \
  }
#define PRINT2(x, y)                                                      \
  {                                                                       \
    std::stringstream msg;                                                \
    msg << STRING(x) << " = " << (x) << ", " << STRING(y) << " = " << (y) \
        << std::endl;                                                     \
    std::cout << msg.str();                                               \
  }
#define PRINT3(x, y, z)                                                   \
...
#define THROW_RUNTIME_ERROR(str)                                 \
  throw std::runtime_error(std::string(__FILE__) + " (" +        \
                           std::to_string((long long)__LINE__) + \
                           "): " + std::string(str))
...
#pragma warning( \
    disable : 265)  // floating-point operation result is out of range
#pragma warning( \
    disable : 383)  // value copied to temporary, reference to temporary used
#pragma warning(disable : 869)  // parameter was never referenced
#pragma warning(disable : 981)  // operands are evaluated in unspecified order
#pragma warning( \
    disable : 1418)  // external function definition with no prior declaration
...
#pragma warning(disable : 4800)  // forcing value to bool 'true' or 'false'
                                 // (performance warning)
#pragma warning(disable : 4267)  // '=' : conversion from 'size_t' to 'unsigned
                                 // long', possible loss of data
#pragma warning(disable : 4244)  // 'argument' : conversion from 'ssize_t' to
...
#pragma warning( \
    disable : 4305)  // 'initializing' : truncation from 'double' to 'float'
#pragma warning(disable : 4068)  // unknown pragma
#pragma warning(disable : 4146)  // unary minus operator applied to unsigned
                                 // type, result still unsigned
#pragma warning(disable : 4838)  // conversion from 'unsigned int' to 'const
